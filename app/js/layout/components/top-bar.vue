	...
	<header class="w-full flex flex-col">
		...
		>
			<div class="flex items-center h-full px-3">
				<button
					class="w-5 mr-5 text-primary-500 flex-shrink-0"
					@click="toggleSidebar"
				>
					<bars-icon></bars-icon>
				</button>
				...
				<PageProvider v-slot="{ meta }">
					<h1
						class="text-primary-100 font-semibold text-sm cursor-default mr-5 transition flex-shrink-0"
					>
						{{ meta.title }}
						<span
							class="text-primary-300 transition-all inline-block transform"
							:class="{
			...
			<div class="flex text-primary-300 flex-shrink-0">
...
.top-bar-button-group {
	@apply flex items-center justify-center text-xs font-semibold text-primary-400 hover:text-primary-300 hover:bg-primary-800 transition-colors h-full px-3 cursor-pointer;
