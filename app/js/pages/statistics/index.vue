<template>
	<main class="dashboard-page p-5 md:p-12">
		<ServiceProvider service="telemetry" v-slot="{ stats }">
			...
			<h2 class="text-xl text-primary-400 font-semibold mb-32 border-t-4 border-primary-800 pt-1 h-0 rounded-full relative shadow-lg opacity-70">
				<span class="bg-primary-800 rounded-b px-2 py-1 shadow-lg">Telemetry</span>
			</h2>
			...
			<h2 class="text-xl text-primary-400 font-semibold mb-32 border-t-4 border-primary-800 pt-1 h-0 rounded-full relative shadow-lg opacity-70">
				<span class="bg-primary-800 rounded-b px-2 py-1 shadow-lg">Metrics</span>
			</h2>
			...
			<h2 class="text-xl text-primary-400 font-semibold mb-32 border-t-4 border-primary-800 pt-1 h-0 rounded-full relative shadow-lg opacity-70">
				<span class="bg-primary-800 rounded-b px-2 py-1 shadow-lg">CPU Cores</span>
			</h2>
		...
		<top-bar-actions>
			...
			<top-bar-button @click="showExampleNotification('error')">
				Example Error
			</top-bar-button>
			<top-bar-button @click="showExampleNotification('warn')">
				Example Warning
			</top-bar-button>
			<top-bar-button @click="showExampleNotification('success')">
				Example Success
			</top-bar-button>
			<top-bar-button @click="showExampleNotification('info')">
				Example Info
			</top-bar-button>
		</top-bar-actions>
	</main>
...
<style lang="scss">
.stat-block {
	@apply leading-tight font-semibold text-primary-50 text-lg mb-5 lg:mb-0;
	...
	.stat-block-heading {
		@apply text-primary-400 text-xs truncate;
	}
}
</style>
...
export default {
	...
	methods: {
		showExampleNotification(type = 'error') {
			this.$notify({
				type: type,
				title: 'Example ' + type,
				text: 'Example body for ' + type,
			});
		}
	}
};
