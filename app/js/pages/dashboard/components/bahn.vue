<template>
	<section class="mt-16">
		<div class="dashboard-title mb-2">Trains to Lübeck Hbf</div>
		<div class="list-none p-0  text-main-lightest inline-block">
			...
			>
				...
				<div class="border-main-darker border-r pr-3 mr-3 text-2xl">
					<div>{{route.departure | dateify}}{{ route.departureDelay ? ` (+${route.departureDelay})` : '' }}</div>
					<div >{{route.arrival | dateify}}{{ route.arrivalDelay ? ` (+${route.arrivalDelay})` : '' }}</div>
				</div>
